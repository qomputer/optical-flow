...
def rem_pad(img, rows):
    nimg = np.delete(img, range(rows[0]), axis = 0)
    mrows = nimg.shape[0]
    crop_img = np.delete(nimg, range(mrows-rows[1],mrows), axis = 0)
    return crop_img

# def rem_pad(img, thresh, rows = 0):
#     if rows == 0:
#         rows = img.shape[0]
#     nimg = np.copy(img)
#     indx = []
#     for i in range(rows):
#         if (np.sum(img[i,...], axis = 1) < thresh).all(0) | (np.sum(img[i,...], axis = 1) > (255-thresh) *3).all(0):
#            indx.append(i)
#     nimg = np.delete(nimg, indx, axis = 0)
#     return nimg

# For debug
# img = cv2.imread("videos/test/frame_0005.png")
# img[0:10] = 0
# im = rem_pad(img, 7)
# cv2.imshow("test", im)
# cv2.waitKey(0)
